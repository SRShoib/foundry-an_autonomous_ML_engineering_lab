...
needs are START -> principal and reporter -> END; everything else is dynamic routing, validated
at compile time by each node's Command[Literal[...]] return annotation (verified against the
installed LangGraph 1.2.9 source: a typo'd goto target raises at .compile(), it is not a silent
failure).
...
def build_graph(checkpointer: BaseCheckpointSaver | None = None) -> CompiledStateGraph:
    ...
    builder.add_node("experiment_runner", experiment_runner)
